...
const tags = {
  ...
  'el-tabs': el => {
    const {
      tag, vModel, clearable, width
    } = attrBuilder(el)
    let child = buildElTabsChild(el)
    if (child) child = `\n${child}\n` // 换行
    return `<${tag} ${vModel} ${clearable} ${width}>${child}</${tag}>`
  },
  'el-radio-group': el => {
...
function buildElTabsChild(scheme) {
  const children = []
  const childs = scheme.__config__.children
  const slot = scheme.__slot__
  if (slot && slot.options && slot.options.length) {
    const child = []
    childs.forEach((item, index) => {
      const str = layouts[item.__config__.layout](item)
      children.push(`<el-tab-pane label="${slot.options[index].title}" name="${slot.options[index].name}">${str}</el-tab-pane>`)
    })
  }
  return children.join('\n')
}
