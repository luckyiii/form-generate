      ...
      <el-scrollbar class="right-scrollbar">
        ...
        <el-form v-show="currentTab === 'form'" size="small" label-width="90px">
          ...
          <el-form-item label="多标签">
            <el-switch v-model="formConf.moreTabs" @change="handleValue" />
          </el-form-item>
          <div v-if="formConf.moreTabs">
            <draggable
                :list="activeData.__slot__.options"
                :animation="340"
                group="tabsItem"
                handle=".option-drag"
              >
                <div v-if="activeData.__config__.tag === 'el-tabs'">
                  <div v-for="(item, index) in activeData.__slot__.options" :key="index" class="input-wraper">
                    <el-input class="input-tab" v-model="item.title" placeholder="标签名" size="small" />
                    <div class="move-wraper">
                      <i class="el-icon-top move-up" v-if="index !== 0" @click="moveUp(index)"></i>
                      <i class="el-icon-bottom move-down" v-if="index !== (activeData.__slot__.options.length - 1)" @click="moveDown(index)"></i>
                    </div>
                    <div class="delete-wraper">
                      <el-button class="delete-btn delete-bton" icon="el-icon-delete" type="text" @click="activeData.__slot__.options.splice(index, 1)"></el-button>
                    </div>
                  </div>
                </div>
              </draggable>
                <div style="text-align: center;">
                  <el-button icon="el-icon-circle-plus-outline" type="text" @click="addTab">添加标签</el-button>
                </div>
            </div>
        </el-form>
      </el-scrollbar>
...
import {
  tabsComponent, inputComponents, selectComponents, layoutComponents
...
export default {
  ...
  methods: {
    ...
    // 节点上移
    moveUp(index) {
      this.activeData.__slot__.options.splice(index - 1, 1,
        ...this.activeData.__slot__.options.splice(index, 1, this.activeData.__slot__.options[index - 1]))
    },
    // 节点下移
    moveDown(index) {
      this.activeData.__slot__.options.splice(index + 1, 1,
    ...
    handleValue() {
      console.log(111)
      this.$emit('transTabsList', this.formConf.moreTabs)
    },
    addSelectItem() {
      this.activeData.__slot__.options.push({
        label: '',
